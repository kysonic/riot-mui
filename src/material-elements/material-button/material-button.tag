<material-button>
    <material-waves onclick="{click}" onmousedown="{launch}" center="{opts.wavesCenter}" rounded="{opts.rounded}" opacity="{opts.wavesOpacity}" color="{opts.wavesColor}"
                    duration="{opts['waves-duration']}" ref="material-waves"></material-waves>
    ...
    <script type="es6">
        ...
        // Launch waves
        this.launch = (e)=>{
            if(!this.disabled) this.refs['material-waves'].trigger('launch',e);
        }
        this.on('mount', ()=>{
          /**
           * When wave will be starting invoke appropriate event
           */
          this.refs['material-waves'].on('wavestart',(wave)=>{
              this.trigger('wavestart',wave);
          });
          /**
           * When wave will end invoke appropriate event
           */
          this.refs['material-waves'].on('waveend',()=>{
